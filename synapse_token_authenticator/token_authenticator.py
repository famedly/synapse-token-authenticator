...
class TokenAuthenticator:
    ...
    def __init__(self, config: TokenAuthenticatorConfig, module_api: ModuleApi):
        ...
        if self.config.jwt:
            if self.config.jwt.secret:
                ...
                if self.config.jwt.keyfile:
                    with open(self.config.jwt.keyfile, "rb") as f:
    ...
    def _verify_jwt(
        ...
        if not key or not isinstance(key, (JWKSet, JWK)):
            logger.info("Invalid key type %s", type(key))
            return None
        try:
            return jwt.JWT(
                jwt=token,
    ...
    async def check_oauth(
        ...
        try:
            if oauth_config.should_use_alternative_fq_uids():
                ...
                # This can raise an TypeError(if the list in the claim we are searching for was not a list), or a
                # ClaimMismatchError if the data is not found in an existing list. If the list just did not exist, it is
                # a None.
                fully_qualified_uid = oauth_config.search_claims_for_alternative_fq_uid(
                    username_fq_uid, jwt_claims, introspection_claims
                )
        ...
        except (TypeError, ClaimsMismatchError, SynapseError) as e:
            logger.info("%s", e)
            return None
        ...
        displayname = None
        try:
            if not oauth_config.should_use_alternative_fq_uids():
                # Always ignore the displayname when using alternative user ids. Otherwise, this user may look like a
                # new person every day!
                displayname = get_reconciled_claim(
                    jwt_claims,
                    introspection_claims,
                    oauth_config.jwt_validation,
                    oauth_config.introspection_validation,
                    "displayname_path",
                )
    ...
    async def check_epa(
        ...
        enc_token = self._verify_jwt(
            token=token_data, key=epa_config.enc_jwk, expected_type="JWE"
        )
        if enc_token is None:
            return None

        token = self._verify_jwt(
            token=enc_token.claims, key=epa_config.jwk_set, check_claims=check_claims
        )
        if token is None:
            return None
        ...
        localpart = get_claim_from_validation(jwt_claims, epa_config, "localpart_path")
        displayname = get_claim_from_validation(
            jwt_claims, epa_config, "displayname_path"
        )
