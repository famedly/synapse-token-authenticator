...
from synapse_token_authenticator.config.oauth import (
...
class TestJwtValidationConfig:
    def test_jwt_validation_config_defaults(self):
        config = JwtValidationConfig(jwk_set=get_jwk())
...
class TestOAuthConfig:
    def test_oauth_config_defaults(self):
        config = OAuthConfig(
            jwt_validation=JwtValidationConfig(jwk_set=get_jwk()),
    ...
    def test_oauth_config_jwt_validation_only(self):
        config = OAuthConfig(jwt_validation=JwtValidationConfig(jwk_set=get_jwk()))
        assert config.jwt_validation is not None
